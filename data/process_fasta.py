...
if __name__ == "__main__":
    ...
    if args.start_date is None:
        setattr(args, "start_date", START_DATE)

    save_path, save_ids_path = customize_save_path(args)

    # Read meta data
    meta_data = read_meta(args.meta_data_path, meta_data_acc_id_field["flu"], delimiter=",")
    if args.end_date is None:
        print("Automatic detected last date:", (datetime.datetime(*get_max_collection_date(meta_data, meta_data_collection_date_field["flu"])) + datetime.timedelta(days=1)).strftime("%Y-%m-%d"))
    ...
    for i, pid in enumerate(tqdm(id2seq)):
        ...
        collection_date = meta_data[seg_id][meta_data_collection_date_field["flu"]]
